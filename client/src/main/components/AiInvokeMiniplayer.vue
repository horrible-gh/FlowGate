...
// it. D0008 §3.6 dropped the 질의응답 branch from this monitor, so the order is live work,
// then the transient end-of-run tone (danger for partial/none/lost, success for a clean
// finish). Unanswered inquiries are the alarm bell's surface now, not the chip's.
...
/* The outline stays neutral in every state — splitting static/running by border would
   double up on a signal the badge already carries (CH0016). The 질의응답 tone that used to
   live here is gone with D0008 §3.6: this monitor no longer speaks about inquiries. */
