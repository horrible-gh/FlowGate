<template>
  ...
  <div ref="rootRef" class="card md-preview-card wp-editor" :class="`wp-layout-${layoutTier}`">
    ...
    <div class="card-bd wp-body">
      ...
      <!-- P0009 ﾂｧ4.5: an unreadable canonical file never gets forced into a table. -->
        ...
        <div v-if="unreadable.revisions?.length" class="wp-unreadable-revisions">
          ...
          <ul>
            ...
            <li v-for="rev in unreadable.revisions" :key="rev.revision_no" class="wp-revision-row">
              ...
              <span>r{{ rev.revision_no }} 窶・{{ rev.created_by }} ﾂｷ {{ rev.created_at }}</span>

              <!-- HEAD (main) had this button as one line, kept verbatim for provenance 窶・
                   ...
                   窶・the same class/:disabled/:title/@click below, split across lines with wp-restore-btn, :class, :aria-label and a restoring-in-progress label added. -->
      ...
      <template v-else-if="plan">
        ...
        <!-- 0403 NR0004 F5 窶・makes sure unsaved edits are not silently erased by the AI fill.
        ...
        <!-- NR0003 rev2 窶・the upload PUT already committed a new revision server-side; this
        ...
        <!-- flowgate.default.0576 TR0005 rev4 窶・PUT /work-plan refuses to apply a body at all
             ...
             until the person reviews the findings and explicitly confirms; [・ｨ・珪 drops it. -->
        ...
        <div v-if="capabilityWarnings.length" class="wp-conflict-banner wp-capability-banner" data-test="capability-warning-banner">
          ...
          <ul class="wp-capability-list">
            ...
            <li v-for="finding in capabilityWarnings" :key="finding.step_key">
              ...
              <strong>{{ finding.step_key }} ﾂｷ {{ finding.step_type }}</strong>

              窶・{{ finding.provider_name || finding.provider_id || '?' }}:
          ...
          <div class="wp-qty-grid">
            ...
            >
              ...
              <span class="wp-qty-stepper">
                ...
                <button type="button" class="wp-stepper-btn" :disabled="isLocked" @click="setQuantity(code, (plan.quantities[code]?.count ?? 0) - 1)">竏・/button>
        ...
        <!-- 竭｡ per-step provider ﾂｷ one-line note -->
            ...
            <template v-for="(step, idx) in plan.steps" v-else :key="step.key">
              ...
              <div class="wp-step-row" :class="{ 'is-first': isFirstOfType(step, idx), 'is-locked': step.locked, 'wp-row-error': stepErrors[step.key] }">
                ...
                >
                  ...
                  <span class="wp-review-pill-text">{{ step.locked ? '窶・ : reviewSummaryText(step) }}</span>
        ...
        <!-- Mockup xc32frrg screen 1 窶・3 quantity cards -->
    ...
    <!-- Raw view overlay (read-only) 窶・P0009 ﾂｧ3.4 / D0007 ﾂｧ6.5.

         flowgate.default.0560 T0018 (4・懍怱, NR0011 ・川棗 ID 45) on the common dialog layer;

         D0008 maps it to `readonly`. It has no footer and gets none (T0018 ﾂｧ2.2-2): its two

         controls always sat in the title bar, so [・ｵ・ｬ] stays there through DialogHeader's

         `actions` slot and the separate [・ｫ・ｰ] button becomes the header X that D0008 ﾂｧ2

         assigns to DialogHeader 窶・the close control keeps its place, it is the common one

         now. `size="lg"` is the same 720px track `.wp-raw-box` had.

         T0018 ﾂｧ2.3-7: like the AI-scope dialog above, this was a non-Teleport
         ...
         `:close-on-backdrop="false"` stays explicit (T0018 ﾂｧ2.2-3): NR0011 records
         ...
         overlay div 窶・it never fired (nothing inside was focused, NR0011 ﾂｧ9-5); the common
...
// 笏笏 Canonical shape (mirrors flowgate.default.0395 P0009 ﾂｧ2 / L0010 ﾂｧ1-2) 笏笏笏笏
...
// T0010 ﾂｧ5.4 / D0007 ﾂｧ5.1 窶・same SSOT pattern as noteMaxChars: the server publishes the
...
// D0007 ﾂｧ6.1-ﾂｧ6.2 窶・the three entry points (step name / review pill / instruction icon) open
...
const props = defineProps<{
  ...
  /** 0424 TR0005 rev2 窶・the document panel's AI-run lock, passed down exactly like
   ...
   *  work plan (save ﾂｷ quantity stepper ﾂｷ provider ﾂｷ note ﾂｷ apply-to-all ﾂｷ AI suggest) stayed clickable
...
// 0403 NR0004 F5 窶・whether there is an unsaved edit. Because the AI fill swaps the screen

// for the server's canonical copy, the AI cannot be handed control while this is true.

const dirty = ref(false)

// 0403 NR0004 F7 窶・the server judges editability and carries it in the response. If the
...
// revisionNo, so editing/saving must stay blocked (via isLocked) until reload() confirms it 窶・
...
// The body is held here, unapplied, until the user explicitly confirms 窶・a manual save and a
...
// T0007 ﾂｧ1.2 窶・the raw {code, params, loc, key, msg} the server sends is the SSOT; the
...
// HEAD (main) had this as one inline type, kept verbatim for provenance 窶・
...
// 窶・every one of those fields is still here, just split into named interfaces below
...
interface WPUnreadable {
  ...
  // The document's own current revision 窶・distinct from each recovery candidate's
...
// D0007 ﾂｧ3.2 decision 4: a value-bearing step that a lower quantity would drop stays
...
// 0424 B0001 / TR0005 rev2 窶・"AI・､嵂餓､卓乱 ・・款・､・ｴ ・壱・・ｬ・・﨑俯腰・ ・・腹・ｼ・ 奝・､孖ｸ ・・垈・ｴ

// ・､・ｸ・?". This group's own [Load AI Suggestion] starts a run against this very WP document
...
// [Download] is blocked when there is nothing to download. main's original guard 窶・
// `!plan.value && !unreadable.value?.raw` 窶・covers the ordinary case; 0599 widened it
...
// The scope dialog is a write surface too 窶・a run that starts while it is open must not
 ...
 * A set row is named after the pair, not after its instruction document 窶・
...
// L0010 ﾂｧ2.9: editor and apply preview share the same measured-width classifier.
...
// 笏笏 Type ordering / expansion (mirrors L0010 ﾂｧ2.1 / ﾂｧ2.2) 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
function hasValue(step: WPStep): boolean {
  ...
  // D0007 ﾂｧ5.5 窶・a step carrying only a new field (review/reviewer/pre-instruction) is not
...
function reexpand(
  ...
  for (const s of fresh) {
    ...
    if (prior && !s.locked) {
      ...
      // D0007 ﾂｧ3.6 / ﾂｧ5.5 窶・a quantity round-trip (down then back up) must not lose the new
...
// 笏笏 Fetch / recovery 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
function applyReadView(data: any) {
  ...
  // response (old response ﾂｷ mock), leave it open 窶・only the server knows whether to
...
// round trip 窶・a strict improvement, not an independent behavior, so it is not kept
...
// `lastApplication` (the [・溢ｧ・・・・圸] line) and the rest of this view stayed on
...
// 笏笏 Quantity editing 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
async function setQuantity(code: string, next: number) {
  ...
  // D0007 ﾂｧ5.5 窶・a quantity decrease that would drop a step still carrying provider/note/
...
// 笏笏 Step editing 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
// D0007 ﾂｧ3.3/ﾂｧ3.4 窶・reviewer is a second, independent provider slot per step. An empty
...
// 笏笏 Review count / reviewer / pre-instruction editing (T0010 ﾂｧ4-ﾂｧ7, D0007 ﾂｧ3.2-ﾂｧ3.4) 笏笏笏笏笏笏
...
// D0007 ﾂｧ3.2 窶・a result-role step (NR/TR) is a report: pre-instruction has no meaning there.

// A locked (TSR) step never reaches this 窶・its drawer never opens at all.
...
function setStepReviewCount(key: string, count: number) {
  ...
  // D0007 ﾂｧ3.3 窶・"・・・・・﨑ｨ・・・・ｸ ・ｨ・・乱・罹株 ・・・・ｴ・ｹ ・ｸ・ｴ ・ｼ・・共": a stale reviewer left
...
// D0007 ﾂｧ3.7 窶・the file goes straight to this document's own attachment store on selection;
...
async function saveFromDrawer(stepKey: string, stepIndex: number) {
  ...
  // 'failed' / 'capability_warning': the drawer and its inputs stay exactly as-is (D0007 ﾂｧ6.2)

  // 窶・the existing save-failure / capability-warning banners already show what went wrong.
 ...
 * "・､・・ｧ・・1・･", "・卓羅 ・逸小孖ｸ 1・ｸ孖ｸ". A report row has no quantity entry of its own,
...
// 笏笏 Scoped project-map suggestion and real AI invocation 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
async function fetchSuggestion(scope: WorkPlanScope) {
  ...
  try {
    ...
    // same code as the save path, so the same [Reload] strip is shown 窶・a toast alone would
...
// 0434 B0001("F5・ｼ ・・･ｴ・ ・喜愍・ｴ ・・圸・們ｧ ・喜搆") 窶・・・onAiInvoke ・・*・ｴ 嶹罷ｩｴ・川・ ・懍梠﨑・

// AI ・・垈・ｰ(startAiFill)・・run_id ・・・幎株・､. ・ｸ・ｰ・ｰ ・､・罹｡・・卓羅・・嚶・・・､・・・ｰ・・・・捩 ・俯､

// ・､ ・奝 ・雅劇・川・ ・懍梠﨑俯株 ・ｸ・ｹ ・護ｻ､・ｴ・, ・ｸ run_id ・・aiRunId ・・・､・ｴ・､・ ・危株・､. ・ｸ・們・

// ・護ｻ､・ ・・・ｬ・・・揆 ・ｱ・晨紛・・・ｴ 嶹罷ｩｴ・ ・ｴ ・・・ｽ・ ・・・・嚶(・俯汢ﾂｷ・ｨ・・ｷ・倆敢)・・・・・ ・ｸ・ｸ・,

// ・罷・・・嚶・ F5 ・､・川幣 ・ｴ・・､ 窶・・ｬ・護擽 ・ｴ・ｰ・・"・們倣毎・､・罷魂 ・・圸・ｴ ・・・・ 嶹罷ｩｴ・ｴ・､.

// ・､・ｸ ・ｸ・・夋・・攪 ・ｸ・ｸ・ ・ｴ・ｸ ・呷捩 ・ｴ・､孖ｸ・・・､・・・ｸ・ｰ・､(MdViewer, ConversationView).

// ・卓羅・・嚶 寬ｸ・滝ｸｰ・・・｣・ ・伎ｳ ・溢来・､. ・罹ｲ・株 ・ｸ・・罹共 窶・・ｸ・菩侃 edit ・ｱ・晧擽 ・ｴ・ｸ

// document_explorer_refresh(operation='updated') ・ｼ ・ｴ・ｴ・ useFlowGateSse ・ ・ｸ・・揆

// fg:document_content_changed ・・・緋ｿ・・｣・罷共. ・・･﨑們ｧ ・喜捩 岺・寬ｸ・卓擽 ・溢愍・ｴ ・､・・・ｽ・

// ・危株・､: ・ｨ・・・・･・ｴ ・ｴ・ ・俾ｳ ・壱腰 ・廷揆 ・ｰ・ｩ德・・ｮ・ｴ・ｰ・ｴ ・・・俾ｳ, ・ｸ ・ｽ・ｰ・・[・・･] ・・
// ・罹ｲ・ｰ 409 ・・・誤､ ・ｼ・・・ｰ・ｴ ・ｩ・・・・ ・俯ｦｬ﨑罹共.
...
// 笏笏 Save (P0009 ﾂｧ4.6 ~ ﾂｧ4.8) 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
function canonicalBody(): WPBody {
  ...
  // T0007 ﾂｧ3 窶・GET can hand back top-level x_* extension fields the server preserves but this
...
// NR0003 ﾂｧ5 窶・the PUT / validation-error / conflict handling that a manual save and a JSON
...
// flowgate.default.0576 TR0005 rev4 窶・the same PUT can also refuse to apply the body at all
...
// 笏笏 Download / upload (flowgate.default.0576 T0004 / NR0003) 笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏笏
...
// NR0003 ﾂｧ3.1 窶・the download source is a fresh GET of the canonical body, never `rawJson`.
...
async function downloadWorkPlan() {
  ...
  // name instead 窶・same [Download]-always-available behavior, honester content type.
...
// NR0003 ﾂｧ4.2 窶・a parsed upload is never written into `plan.value` before the PUT succeeds.
...
// upload failure 窶・it only means this screen could not confirm it, so fetchPlan's own error
...
// dirty, while plan.value is still the pre-upload body 窶・saving from that mismatched state would
...
// flowgate.default.0576 TR0005 rev4 窶・the explicit confirmation the capability gate requires

// (P0009 / provider_capability_service): re-send the same held body with every finding's

// step_key acknowledged. Never auto-derived from a click elsewhere 窶・only this button counts.
...
async function confirmCapabilityWarning() {
  ...
  try {
    ...
    if (result.status === 'capability_warning') {
      ...
      // The findings changed underneath (providers/steps moved) 窶・show the current set instead
...
/* rej_01M2VY4Z8FG9M2S0 窶・card-hd's .card-actions has no gap rule in this component (unlike
...
/* 0403 NR0004 F5 窶・unsaved-edit strip. Same spot, same shape as the save-conflict strip. */
...
/* Mockup xc32frrg screen 1 窶・quantity card: tag ﾂｷ name/unit ﾂｷ stepper on one line. */
...
/* T0010 ﾂｧ6 / ・懍譜 deck t17hbdfg v8 窶・step-name button, review pill, instruction icon: the

   three entry points that open the same per-step drawer (D0007 ﾂｧ6.1). */
...
/* Mockup xc32frrg screen 1 窶・bottom 3 quantity cards */
...
/* main's original rule, superseded by 0599's wider version directly below (same selector,
   ...
   later one wins the cascade) 窶・kept so the pre-0599 sizing is still visible in history. */
...
/* flowgate.default.0576's row rule, kept alongside 0599's `li` rule above 窶・the row still
...
/* The overlay, the box and the title row belong to the common dialog layer now (T0018);
   ...
   `display:flex; flex-direction:column; overflow:hidden; padding:0` column 窶・the same box

   `.wp-raw-box` was 窶・so the dark block still bleeds to the surface edge and is still the one
...
/* L0010 ﾂｧ1.4: mid tier drops the document column, narrow tier stacks cards. */
