<template>
  <!-- flowgate.default.0560 T0022 §2.3 (5순위) - migrated onto the common dialog layer.
       D0008 §6 maps this instance to `workflow-large`. `surface="panel"` is an explicit caller
       value (`sheet`'s fixed `min(860px, 88vh)` would stretch a content-height picker), and
       `size="xl"` is the nearest track to the 940px `.modal-wpc` carried - `lg` (720) would
       squeeze the two-column grid, the reason T0016 gave for `WorkflowDecisionModal`.

       ESC and initial focus are the stack's now. The overlay used to carry a ref of its own
       + a tabindex of -1 + an escape-keydown binding, and `watch(visible)` ended with
       `setTimeout(() => overlayRef.value?.focus(), 50)` - a key handler bound to an element
       only fires while that element holds focus, so the box had to grab focus to hear ESC at
       all. L0009 §2 puts exactly ONE document keydown listener in the stack manager, which is
       focus-independent, and L0009 §4 decides where focus starts; keeping the old wiring would
       mean two handlers for one key. `aria-labelledby` is generated by `DialogShell` and
       pointed at `DialogHeader`'s title, so the hand-written `id="wpc-title"` went too.

       `closeOnBackdrop` is not overridden: `workflow-large` defaults to `false`, and clicking
       the old `.modal-bg` never closed this dialog either (it had no self-click handler). -->
  <DialogShell
    ref="shellRef"
    :open="visible"
  ...
  >
    <template #header>
      <DialogHeader :title="t('main.work_plan_create_dialog.title')" @close="onHeaderClose">
        <template #icon>
            ...
            <section class="wpc-sec">
              <div class="wpc-sec-hd">
                ...
                <span class="wpc-sec-acts">
                  <!-- flowgate.default.0560 TR0023 rejection rework: the primary [만들기] button
                       is disabled until a type and a provider are picked, so on a cold open
                       `DialogShell`'s tree would nominate it and then no-op `.focus()` on a
                       disabled element, leaving focus outside the dialog. [전체선택] is the
                       first control in reading order that is always enabled, so it is the
                       intentional initial-focus target instead. -->
                  <button type="button" class="wpc-mini-btn" data-dialog-autofocus @click="selectAllTypes">
    ...
    <template #footer>
      <!-- The slot carries the `plus` icon and nothing else: the busy/idle label swap lives in
           `DialogAction.label` (a computed), because a label can change without a slot and this
           button - unlike `AiInvokeDialog`'s - keeps the same icon in both states. -->
...
watch(
  ...
  (val) => {
    ...
    void loadTypes()
    void loadProviders()
    // T0022 §2.3: `setTimeout(() => overlayRef.value?.focus(), 50)` is gone. It existed to let
    // the overlay hear the element-bound ESC handler; the stack's single document listener has
    // no such requirement, and L0009 §4's tree focuses a control INSIDE the surface on the
    // nextTick after open instead of the overlay box 50ms later.
  },
  { immediate: true },
)
...
const createLabel = computed(() =>
  creating.value
    ? t('main.work_plan_create_dialog.creating')
    : t('main.work_plan_create_dialog.create'),
)

/**
 * T0022 §2.3 - `[취소](cancel)` and `[만들기](primary)`, both `disabled` exactly as the two
 * inline buttons were.
 */
const actions = computed<DialogAction[]>(() => [
  ...
  },
  {
    ...
    role: 'primary',
    disabled: !!blockReason.value || creating.value,
    onSelect: onCreate,
  },
])
...
/* Layout mirrors mockup xc32frrg screen 2 (work plan create dialog): two columns
   ...
   footer out of reach. */
/* `.modal-wpc` left with the markup (the width is the shell's `size="xl"`), and `.wpc-body`'s
   padding with it - `panel`'s body rule already pads 18px 20px and scrolls. */
.wpc-title-ico { color: var(--primary, #4f46e5); }
.wpc-body { display: flex; flex-direction: column; gap: 12px; }
.wpc-intro {
