...
{path:'',redirect:'/settings/ai-run-monitor'},{path:'ai-run-monitor',component:AiRunMonitorSettingsView},{path:'security',component:SecuritySessionsView},
...
const router=createRouter({history:createWebHistory(),routes});router.beforeEach(to=>{const auth=useAuthStore();if(to.meta.requiresAuth&&!auth.isAuthenticated){window.location.href='/index.html';return false}if(to.meta.permission&&!auth.can(to.meta.permission))return {path:'/settings/ai-run-monitor'};return true});export default router;
