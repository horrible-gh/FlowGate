...
describe('MainPanel AI-run rendered presentation (0398)', () => {
  ...
  it('fails closed during bootstrap, then renders the sticky card with locked readable content', async () => {
    ...
    const workflow = wrapper.findComponent({ name: 'DocWorkflow' })
    expect(workflow.exists()).toBe(true)
    expect(workflow.props('readOnly')).toBe(true)
    expect((workflow.props('stepStates') as Array<{ code: string }>).map(step => step.code))
      .toEqual(['R', 'T', 'TR', 'AC'])
    const statusCard = wrapper.find('.ai-invoke-status-card')
    const docHeader = wrapper.find('.doc-header')
    expect(statusCard.element.compareDocumentPosition(docHeader.element) & 4).not.toBe(0)
    expect(docHeader.element.compareDocumentPosition(workflow.element) & 4).not.toBe(0)
