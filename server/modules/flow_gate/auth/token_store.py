...
def blacklist_token(jti,user_id,exp_timestamp):get_store()._execute("INSERT OR IGNORE INTO token_blacklist (jti,user_id,revoked_at,expires_at) VALUES (?,?,?,?)",[jti,user_id,now_iso(),_iso(datetime.fromtimestamp(exp_timestamp,timezone.utc))])
def is_blacklisted(jti):return get_store()._fetch_one("SELECT jti FROM token_blacklist WHERE jti=?",[jti]) is not None
