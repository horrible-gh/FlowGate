...
_TS_AUTHORING_TEXT = {
    "ko": (
        ...
        "테스트 목적은 명령 실행 성공이 아니라 요구사항의 기능 결과 검증입니다. 현재 legacy 실행기는\n"
        "종료코드로 판정하므로 cmd 자체가 stdout/JSON/응답 값을 assertion하여 불일치 시 실패해야 합니다.\n"
        "종료코드 0은 실행 성공일 뿐 기능 정합성의 증거가 아닙니다. 기대값을 실제값에 맞춰 약화하지\n"
        "말고, 결함이 의심되면 제품 코드 수정의 검증 근거가 되게 하십시오. 기대 필드는 향후 native\n"
        "assertion에 적용할 구체적 결과로 쓰되 현재 prose 기대를 기계 비교하지는 않습니다. pytest,\n"
        "npm, go, cargo, assertion 스크립트 등 대상에 맞는 러너를 쓰십시오. assert 필드가 없는 기존\n"
        "TS 형식도 계속 지원됩니다.\n\n"
        "{shell_guidance}"
    ...
    "ja": (
        ...
        "テスト目的はコマンド実行の成功ではなく、要件どおりの機能結果を検証することです。現在の\n"
        "legacy実行器は終了コードで判定するため、cmd自体がstdout/JSON/応答値をassertし、不一致なら\n"
        "失敗する必要があります。終了コード0は実行成功だけを示し、機能の正しさの証拠ではありません。\n"
        "期待値を実際値に合わせて弱めず、欠陥が疑われる場合は製品コード修正の検証根拠にします。\n"
        "expectは将来native assertionへ適用できる具体的結果として書きますが、現在prose expectは\n"
        "機械比較しません。assertなしの従来TSも引き続き動作します。\n\n"
        "{shell_guidance}"
    ...
    "en": (
        ...
        "The legacy runner uses exit codes, so cmd itself must assert stdout, JSON, or response\n"
        "values and exit nonzero on mismatch. Exit 0 alone proves only process execution, not feature\n"
        "correctness. Never weaken an expected value to match the actual result; suspected defects\n"
        "should provide evidence for fixing product code. Write expect as a specific result suitable\n"
        "for future native assertion, although prose expect is not yet machine-compared. Any suitable\n"
        "runner may be used; legacy TS without assert remains valid.\n\n"
        "{shell_guidance}"
