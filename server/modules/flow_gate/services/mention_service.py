...
_TS_AUTHORING_TEXT = {
    "ko": (
        ...
        "## 테스트 케이스       (필수; 최소 한 개)\n"
        "### TC-1: <제목>\n"
        "- cmd: <요구사항의 실제 결과를 검사하고 불일치 시 0이 아닌 코드로 끝나는 한 줄 명령>\n"
        "- 기대: <검증할 구체적 기대 결과; 예: HTTP 200이고 JSON 필드 enabled가 true>\n"
        "- assert: <mode>:<spec>                          # 선택; native 비교로 판정(아래 6개 mode)\n\n"
        ...
        "assert 필드는 다음 6개 mode 중 하나로 native 비교합니다(mode 이름은 번역하지 않음):\n"
        "- exit_code:<N>                       예: exit_code:0\n"
        "- stdout_equals:<literal>             예: stdout_equals:ok\n"
        "- stdout_contains:<substring>         예: stdout_contains:\"enabled\":true\n"
        "- stdout_not_contains:<substring>\n"
        "- json_equals:<dotted.path>=<value>   예: json_equals:data.enabled=true\n"
        "- json_subset:<json-object-literal>   예: json_subset:{\"enabled\": true}\n\n"
        "예 — curl로 JSON 응답의 기능 결과를 검증:\n"
        "### TC-2: enabled 플래그가 true로 응답한다\n"
        "- cmd: curl -s http://127.0.0.1:{PORT}/status\n"
        "- 기대: JSON 응답의 data.enabled가 true\n"
        "- assert: json_equals:data.enabled=true\n\n"
        ...
        "제한: 케이스 최대 50개, 준비/정리 단계 최대 20개, 서비스 최대 5개.\n\n"
        "테스트 목적은 명령 실행 성공이 아니라 요구사항의 기능 결과 검증입니다. assert 필드를 쓰면\n"
        "native 비교(위 6개 mode)가 그 케이스의 판정을 대체합니다. assert 필드가 없는 케이스는\n"
        "여전히 종료코드로만 판정되므로, 그런 케이스는 cmd 자체가 stdout/JSON/응답 값을 assertion하여\n"
        "불일치 시 0이 아닌 코드로 끝나야 합니다.\n"
        "종료코드 0은 실행 성공일 뿐 기능 정합성의 증거가 아닙니다. 기대값을 실제값에 맞춰 약화하지\n"
        "말고, 결함이 의심되면 제품 코드 수정의 검증 근거가 되게 하십시오. 기대 필드는 assert가\n"
        "검사하는 내용을 사람이 읽을 수 있게 서술하되, prose\n"
        "자체는 기계 비교되지 않습니다. pytest, npm, go, cargo, assertion 스크립트 등 대상에 맞는\n"
        "러너를 쓰십시오. assert 필드가 없는 기존 TS 형식도 계속 지원됩니다.\n\n"
        "{shell_guidance}"
    ...
    "ja": (
        ...
        "## Test Cases       (必須; 最低1件)\n"
        "### TC-1: <タイトル>\n"
        "- cmd: <要件の実際の結果を検査し、不一致なら0以外で終了する1行コマンド>\n"
        "- expect: <検証する具体的な期待結果; 例: HTTP 200かつJSONのenabledがtrue>\n"
        "- assert: <mode>:<spec>                          # 任意; native比較で判定(下記6モード)\n\n"
        "## Teardown        (任意; 失敗しても常に実行)\n"
        "- cmd: <後始末コマンド>\n\n"
        "従来の韓国語のセクション名とフィールド名も引き続き受け付けますが、\n"
        "このロケールでは上記の英語表記を使用してください。\n\n"
        "assertフィールドは次の6モードのいずれかでnative比較します(モード名は翻訳しません):\n"
        "- exit_code:<N>                       例: exit_code:0\n"
        "- stdout_equals:<literal>             例: stdout_equals:ok\n"
        "- stdout_contains:<substring>         例: stdout_contains:\"enabled\":true\n"
        "- stdout_not_contains:<substring>\n"
        "- json_equals:<dotted.path>=<value>   例: json_equals:data.enabled=true\n"
        "- json_subset:<json-object-literal>   例: json_subset:{\"enabled\": true}\n\n"
        "例 — curlでJSON応答の機能結果を検証:\n"
        "### TC-2: enabledフラグがtrueで応答する\n"
        "- cmd: curl -s http://127.0.0.1:{PORT}/status\n"
        "- expect: JSON応答のdata.enabledがtrue\n"
        "- assert: json_equals:data.enabled=true\n\n"
        ...
        "上限: ケース最大50件、setup/teardownステップ最大20件、サービス最大5件。\n\n"
        "テスト目的はコマンド実行の成功ではなく、要件どおりの機能結果を検証することです。ケースに\n"
        "assertフィールドがあれば、そのnative比較(上記6モード)がそのケースの判定を置き換えます。\n"
        "assertのないケースは引き続き終了コードのみで判定されるため、そのcmd自体がstdout/JSON/\n"
        "応答値をassertし、不一致なら0以外で終了する必要があります。終了コード0は実行成功だけを\n"
        "示し、機能の正しさの証拠ではありません。期待値を実際値に合わせて弱めず、欠陥が疑われる\n"
        "場合は製品コード修正の検証根拠にします。expectはassertが検証する内容を人が読める形で\n"
        "書きますが、prose自体は機械比較されません。assertなしの従来TSも引き続き動作します。\n\n"
        "{shell_guidance}"
    ...
    "en": (
        ...
        "### TC-1: <title>\n"
        "- cmd: <single-line command that checks the required result and exits nonzero on mismatch>\n"
        "- expect: <specific expected result; e.g. status 200 and JSON field enabled is true>\n"
        "- assert: <mode>:<spec>                          # optional; native comparison (6 modes below)\n\n"
        ...
        "The assert field natively compares one of these 6 modes (mode names are never translated):\n"
        "- exit_code:<N>                       e.g. exit_code:0\n"
        "- stdout_equals:<literal>             e.g. stdout_equals:ok\n"
        "- stdout_contains:<substring>         e.g. stdout_contains:\"enabled\":true\n"
        "- stdout_not_contains:<substring>\n"
        "- json_equals:<dotted.path>=<value>   e.g. json_equals:data.enabled=true\n"
        "- json_subset:<json-object-literal>   e.g. json_subset:{\"enabled\": true}\n\n"
        "Example — verify a curl JSON response's functional result:\n"
        "### TC-2: enabled flag responds true\n"
        "- cmd: curl -s http://127.0.0.1:{PORT}/status\n"
        "- expect: JSON response's data.enabled is true\n"
        "- assert: json_equals:data.enabled=true\n\n"
        ...
        "Limits: at most 50 cases, 20 setup/teardown steps, 5 services.\n\n"
        "The test goal is to verify required functional results, not merely command execution.\n"
        "When a case has an `assert` field, its native comparison (the 6 modes above) replaces\n"
        "the legacy exit-code verdict for that case. A case without `assert` is still judged by\n"
        "exit code alone, so its cmd itself must assert stdout, JSON, or response values and exit\n"
        "nonzero on mismatch. Exit 0 alone proves only process execution, not feature correctness.\n"
        "Never weaken an expected value to match the actual result; suspected defects should provide\n"
        "evidence for fixing product code. Write expect as the human-readable description of what\n"
        "assert checks; prose itself is not machine-compared. Any suitable runner may be used;\n"
        "legacy TS without assert remains valid.\n\n"
        "{shell_guidance}"
