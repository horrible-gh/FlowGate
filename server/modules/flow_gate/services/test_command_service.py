...
def _reflect_one(project: str, cmd: str, desc: str, now: str) -> None:
    ...
    if row is not None:
        ...
        db.update_row(project, row["id"], {"last_success_at": now})  # origin/description untouched
        return
    if db.count_active(project) >= MAX_COMMANDS_PER_PROJECT:
        logger.info("test-command auto-register skipped (list full): %s / %s", project, cmd)
        return
    db.insert(project, cmd, _truncate(desc, DESCRIPTION_MAX_LEN), "auto", now, status="active")
...
def build_verified_commands_block(project: str) -> str:
    ...
    if not rows:
        return ""
    shown = rows[:MENTION_MAX_ITEMS]
    hidden = len(rows) - len(shown)
    lines = [
        "The commands below have been registered for this project (auto entries were",
        "verified by a previous successful remote test run). Prefer these over guessing:",
        "",
    ]
    for row in shown:
        suffix = ""
