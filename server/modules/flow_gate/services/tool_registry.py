...
CAUTIONS = {
    ...
    "merge_preview": {
        "ko": ["git merge-tree --write-tree만 쓰며 HEAD/index/working tree를 절대 바꾸지 않는다(내부적으로 object database에 병합 결과 tree/blob을 남길 수 있으나 어떤 ref에도 연결되지 않는다).", "clean=false일 때만 conflicts에 파일이 들어 있고, conflicts가 200(_MAX_CONFLICT_FILES)을 넘으면 잘리고 truncated=true가 된다.", "이 조회는 실제 merge를 수행하는 것이 아니라 미리보기다 — 실제로 병합하려면 이 도구가 아니라 사람의 개입 또는 다른 절차가 필요하다.", "merge_tree는 후보 병합 결과의 tree oid다. clean 여부와 무관하게 read/grep/glob의 ref에 그대로 넣으면 그 tree 기준 병합 결과를 조회할 수 있으며, 충돌 파일에는 <<<<<<< / ======= / >>>>>>> 마커가 포함된다.", "충돌 경로 하나를 다섯 관점에서 한 번에 볼 수 있다 — 현재 worktree: read(path)(ref 생략, 미커밋 변경 포함) / 작업 브랜치 committed HEAD: read(path, ref=head) / target·main: read(path, ref=target_sha) / 공통 base: read(path, ref=merge_base) / 가상 merge 결과: read(path, ref=merge_tree). 이 응답의 head/target_sha/merge_base/merge_tree를 그대로 ref에 넣으면 되고, 동일한 ref를 grep/glob/stat에도 그대로 쓸 수 있다."],
        "ja": ["git merge-tree --write-treeのみを使い、HEAD/index/working treeを絶対に変更しない(内部的にobject databaseへ統合結果のtree/blobを残す場合があるが、どのrefにも接続されない)。", "clean=falseのときのみconflictsにファイルが入り、conflictsが200(_MAX_CONFLICT_FILES)を超えると切り詰められてtruncated=trueになる。", "この参照は実際にmergeを行うものではなくプレビューである — 実際に統合するにはこのツールではなく人の介入または別の手順が必要である。", "merge_treeは候補統合結果のtree oidである。clean可否にかかわらずread/grep/globのrefへそのまま指定すると、そのtree基準の統合結果を参照でき、conflictファイルには<<<<<<< / ======= / >>>>>>>マーカーが含まれる。", "衝突パス1つを5つの視点で一度に確認できる — 現在のworktree: read(path)(ref省略、未commit変更を含む) / 作業ブランチのcommitted HEAD: read(path, ref=head) / target・main: read(path, ref=target_sha) / 共通base: read(path, ref=merge_base) / 仮想merge結果: read(path, ref=merge_tree)。このレスポンスのhead/target_sha/merge_base/merge_treeをそのままrefに渡せばよく、同じrefはgrep/glob/statにもそのまま使える。"],
        "en": ["Uses only git merge-tree --write-tree and never changes HEAD, the index, or the working tree (it may leave merged tree/blob objects in the object database internally, but none are reachable from any ref).", "conflicts is populated only when clean=false; if it exceeds 200 (_MAX_CONFLICT_FILES) it is truncated and truncated=true.", "This is a preview, not an actual merge -- actually merging requires human intervention or a different procedure, not this tool.", "merge_tree is the candidate merge result tree oid. Regardless of whether the merge is clean, pass it directly as the ref to read/grep/glob to inspect that tree's merge result; conflicted files include <<<<<<< / ======= / >>>>>>> markers.", "See a conflicted path from five angles at once -- current worktree: read(path) (ref omitted, includes uncommitted changes) / worker branch's committed HEAD: read(path, ref=head) / target/main: read(path, ref=target_sha) / common base: read(path, ref=merge_base) / candidate merge result: read(path, ref=merge_tree). Pass this response's head/target_sha/merge_base/merge_tree straight through as ref, and the same ref works for grep/glob/stat too."],
    },
    "read": {
...
def _request_fields(name: str, locale: str) -> list[dict]:
    ...
    if name in {"read", "grep", "glob", "stat"}:
        descriptions = {
            "ko": "선택적 커밋 ref. 생략 또는 null이면 working tree(미커밋 변경 포함), 지정하면 committed tree를 읽는다. merge_preview가 반환한 head/target_sha/merge_base/merge_tree를 그대로 이 ref에 쓸 수 있다.",
            "ja": "任意のcommit ref。省略またはnullはworking tree(未commit変更を含む)、指定時はcommitted treeを読む。merge_previewが返すhead/target_sha/merge_base/merge_treeをそのままこのrefに使える。",
            "en": "Optional committed-tree ref. Omit or send null for the working tree (including uncommitted changes); specify it to read a committed tree. The head/target_sha/merge_base/merge_tree values returned by merge_preview can be passed here directly.",
        }
