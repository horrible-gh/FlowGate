...
``ai_invoke_service.py`` and the normally-imported modules it re-exports from
(``ai_invoke_worker.py`` / ``ai_invoke_provider_api.py`` / ``ai_invoke_provider_cli.py`` /
``ai_invoke_chain.py`` / ``ai_invoke_review.py`` -- flowgate.default.0501 T4 re-split
``ai_invoke_part2_worker.py`` into the first three, T5 re-split ``ai_invoke_part3_chain.py``
into the last two and retired the exec()-assembly scheme entirely), which call the
functions below as the one place that knows how to read the registry, instead of each
inlining its own ``with _runs_lock: ...`` block.
