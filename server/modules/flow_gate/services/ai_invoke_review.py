# ────────────────────── ai_invoke: review / rework orchestration ──────────────────────
#
# flowgate.default.0501 T5: split out of ai_invoke_part3_chain.py (flowgate.default.0497
# T0009's part 3 of 3), alongside ai_invoke_chain.py. A normal Python module — no exec()
# assembly, no dynamic re-export. Non-monkeypatched core constants are a plain
# `from .ai_invoke_service import (...)` at this file's top, and the pure helpers this file
# shares byte-for-byte with `ai_invoke_helpers.py` (`_map_lookup` / `_review_key` / ...) are
# imported directly from there, same as before this split.
#
# Holds: the review gate itself (resolve_review_gate / run_review_gate), reviewer/rework-
# executor selection (resolve_reviewer / resolve_step_executor), review-count/round-limit
# policy (resolve_review_count / resolve_round_limit / review_rounds_remain), the
# review/rework hop launchers (_spawn_review_hop / _spawn_rework_hop), automatic rejection
# on an `issues` verdict (build_auto_reject_reason / _auto_reject), and the document-scoped
# review loop (compute_review_baseline / resolve_document_review_loop_gate /
# _checkpoint_document_review_loop / document_review_loop_payload). `run_review_gate`
# reaches back into ai_invoke_chain.py's handoff primitives (`svc._park_handoff`,
# `svc.clear_auto_resume`, `svc.request_auto_resume`, `svc._settle_gate_pass`'s own
# `svc._clear_handoff_row`) to act once it has decided what happens next, through
# `svc.<name>` (`svc` = ai_invoke_service, imported once below) — T0012 §8 forbids a direct
# ai_invoke_chain <-> ai_invoke_review import in either direction (ai_invoke_chain.py
# already imports this module directly for the forward calls it needs), so this reverse
# handful keeps the one runtime-lookup indirection the split's compatibility seams also use,
# rather than creating the cycle. Everything this file needs from ai_invoke_service.py's own
# core that isn't a plain constant (`_provider_name_of`, `compute_review_baseline`'s own
# callers in start_run, ...) reaches it the same explicit way from the other side: that
# module's own tail carries one named facade entry per name, not a dynamic hook.
#
# Deliberately NOT here: provider transport (HTTP/CLI) details, subprocess handling,
# group-lease repository internals, or the runtime registry owner (ai_invoke_runtime.py's
# _runs/_runs_lock ownership) — this module reaches `svc.<name>` for all of that.
...
def resolve_review_gate(bundle: dict) -> dict:
    ...
    try:
        ...
        logger.warning("review gate could not read reviews for %s", slot["doc_id"],
                       exc_info=True)
        svc._log_review_annotation_failure("read", slot, bundle, exc)
...
def _auto_reject(slot: dict, review: Optional[dict], bundle: dict) -> dict:
    ...
    try:
        ...
        svc._log_review_annotation_failure("write", slot, bundle, exc)
    ...
    if "document.reject" not in permissions:
        ...
        svc._log_review_annotation_failure("write", slot, bundle, detail)
...
def _settle_gate_pass(group_id: str, slot: dict, bundle: dict, run: dict) -> str:
    ...
    svc._park_handoff(run, bundle, stop_code)
    return outcome
...
def _queue_gate_bundle(group_id: str, bundle: dict) -> None:
    ...
    """
    svc.request_auto_resume(group_id, bundle)
...
def run_review_gate(group_id: str, bundle: dict, run: dict) -> bool:
    ...
    if gate.get("reject_first") and slot is not None:
        result = svc._auto_reject(slot, _latest_review_of(slot), bundle)
        if not result.get("ok"):
            run["review_reject_detail"] = result.get("detail")
            svc._park_handoff(run, bundle, result["stop_code"])
            return False

    stage = gate.get("stage")
    if stage == "stop":
        svc._park_handoff(run, bundle, gate.get("stop_code") or HOP_HANDOFF_FAILED_STOP_CODE)
        return False

    if stage == WORK_HOP_KIND:
        if gate.get("approve_first") and slot is not None:
            if svc._settle_gate_pass(group_id, slot, bundle, run) != "continue":
                return False
        ...
        svc._spawn_auto_resume(group_id, {**bundle, "last_stage": WORK_HOP_KIND})
        return True
    ...
    if stage in (REVIEW_HOP_KIND, REWORK_HOP_KIND):
        ...
        try:
            if stage == REVIEW_HOP_KIND:
                svc._spawn_review_hop(group_id, queued, gate)
            else:
                svc._spawn_rework_hop(group_id, queued, gate)
        except Exception:
            svc.clear_auto_resume(group_id)     # take the intent back out; the caller parks it
            raise
        return True
...
def _checkpoint_document_review_loop(run: dict) -> dict | None:
    ...
    with svc.get_store().transaction():
...
def _checkpoint_document_review_loop_tx(run: dict) -> dict | None:
    ...
    if (
        ...
        rejection = svc._auto_reject(slot, latest_review, {
