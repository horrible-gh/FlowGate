...
_ITEM_NOTES: dict[str, dict[str, str]] = {
    "ko": {
        ...
        "design_template_children": "default_child 는 이번에 작성할 문서의 타입입니다. 특별한 이유가 없으면 그것을 받으세요.",
        "design_template_body": "rendered 는 예전 지시문의 '## Document template' 절과 같은 완성 블록입니다. 본문에는 파일 경로가 들어가지 않습니다.",
        "design_template_fallback": "resolved_locale 이 requested_locale 과 다르면 본문은 대체 로케일의 것입니다.",
    ...
    "en": {
        ...
        "design_template_children": "default_child is the type you are writing now. Take that one unless you have a reason not to.",
        "design_template_body": "rendered is the finished block that used to be the '## Document template' section of the mention. The body never contains a file path.",
        "design_template_fallback": "When resolved_locale differs from requested_locale, the body is the fallback locale's.",
    ...
    "ja": {
        ...
        "design_template_children": "default_child は今回作成する文書のタイプです。特別な理由が無ければそれを取得してください。",
        "design_template_body": "rendered は以前の指示文の '## Document template' 節と同じ完成ブロックです。本文にファイルパスは入りません。",
        "design_template_fallback": "resolved_locale が requested_locale と異なる場合、本文は代替ロケールのものです。",
