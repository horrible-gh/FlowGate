...
def _contract_rule_lines(locale: str) -> list[str]:
    ...
    if locale == "en":
        return [
            ...
            "This same schema is also published as data in this item's `contract` field, and `example` is a minimal body that passes validate() unchanged.",
        ]
    if locale == "ja":
        return [
            ...
            "同じ契約はこの項目の `contract` フィールドにもデータとして載っており、`example` は validate() をそのまま通る最小の本文です。",
        ]
    return [
        ...
        "같은 계약이 이 항목의 `contract` 필드에도 데이터로 실려 있고, `example`은 validate()를 그대로 통과하는 최소 본문입니다.",
    ]
