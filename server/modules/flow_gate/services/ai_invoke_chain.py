# ────────────────────── ai_invoke: chain orchestration ──────────────────────
#
# flowgate.default.0501 T5: split out of ai_invoke_part3_chain.py (flowgate.default.0497
# T0009's part 3 of 3), alongside ai_invoke_review.py. A normal Python module — no exec()
# assembly, no dynamic re-export. Non-monkeypatched core constants are a plain
# `from .ai_invoke_service import (...)` at this file's top. Forward calls into
# ai_invoke_review.py that no test monkeypatches (`resolve_review_gate` / `run_review_gate` /
# ...) go through a direct `from . import ai_invoke_review` and
# `ai_invoke_review.<name>(...)` — the two of ai_invoke_worker.py's names this file needs
# (`finished_payload` / `is_resumable`) the same way, via `from . import ai_invoke_worker`.
# The handful of names an existing test reaches with `monkeypatch.setattr(svc, "<name>",
# fake)` (`svc` = ai_invoke_service, imported once below) — `start_run`, and
# `ai_invoke_review`-owned `_spawn_review_hop` / `_spawn_rework_hop` — stay on `svc.<name>`,
# the one runtime-lookup indirection this split keeps.
#
# Holds: run status/cancel, pause/resume and the global active-run/paused-chain list
# (get_status / cancel_run / pause_run / active_all), the per-hop re-spawn and durable
# handoff bookkeeping for unmanned continuous chains (_maybe_auto_resume_hop /
# _spawn_auto_resume / _write_handoff_row / _park_handoff / peek_pop_clear_auto_resume),
# the human resume/release entry points (resume_chain /
# release_paused_chain), and SSE broadcast (_broadcast). It asks ai_invoke_review.py
# (via `ai_invoke_review.resolve_review_gate` / `ai_invoke_review.run_review_gate` /
# `svc._spawn_review_hop` / `svc._spawn_rework_hop`) whenever a step boundary might need a
# review or rework hop instead of a plain work hop, and acts on the answer with its own
# spawn/park primitives.
#
# Deliberately NOT here: review-hop state, findings/verdicts, rework provider/executor
# selection, or the document-review-loop state machine — all ai_invoke_review.py's, called
# through `svc.<name>` the same way. No provider transport (HTTP/CLI) details either.
...
def get_status(run_id: str) -> dict:
    ...
    if run.get("completion_oracle") is None:
        ...
        try:
            docs_so_far = len(svc._oracle_new_docs(run))
        except Exception:
            pass
    ...
    return {
        ...
        "pending_q_doc_ids": svc._open_q_doc_ids(run["group_id"]),
...
def pause_run(run_id: str, user_id: str) -> dict:
    ...
    docs_reached = 0
    try:
        docs_reached = len(svc._oracle_new_docs(run))
    except Exception:
...
def has_active_run(group_id: Optional[str]) -> bool:
    ...
    if not group_id:
        return False
    return svc._active_run_for_group(group_id) is not None
...
def request_auto_resume(group_id: Optional[str], payload: dict) -> None:
    ...
    with _auto_resume_lock:
        svc._auto_resume[group_id] = dict(payload)
    svc._write_handoff_row(group_id, payload, svc._active_run_for_group(group_id))
    ...
    try:
        ...
        db_paused.upsert(
            ...
            stop_last_message_excerpt=(
                ai_invoke_review._review_no_verdict_excerpt(run) if stop_code == REVIEW_NO_VERDICT_STOP_CODE
...
def _resettle_stop_after_park(run: dict, stop_code: str) -> None:
    ...
    run["resumable"] = ai_invoke_worker.is_resumable(stop_code)
    run["stop_reason"] = ai_invoke_worker._stop_reason_text(stop_code, run)
    ...
    ai_invoke_worker._notify_chain_failure_if_needed(
        run,
...
def _park_handoff(run: dict, pending: dict, stop_code: str) -> None:
    ...
    run["stop_code"] = stop_code
    run["resumable"] = ai_invoke_worker.is_resumable(stop_code)
    svc._write_handoff_row(group_id, pending, run, stop_code=stop_code)
...
def peek_auto_resume(group_id: Optional[str]) -> Optional[dict]:
    ...
    if not group_id:
        return None
    with _auto_resume_lock:
        return svc._auto_resume.get(group_id)
...
def pop_auto_resume(group_id: Optional[str]) -> Optional[dict]:
    if not group_id:
        return None
    with _auto_resume_lock:
        return svc._auto_resume.pop(group_id, None)
...
def clear_auto_resume(group_id: Optional[str]) -> None:
    if not group_id:
        return
    with _auto_resume_lock:
        svc._auto_resume.pop(group_id, None)
...
def _maybe_auto_resume_hop(run: dict) -> None:
    ...
    if run.get("end_reason") != "exited" or cancelled:
        ...
        svc._park_handoff(run, pending, parked_code)
        return
    ...
    svc._write_handoff_row(group_id, pending, run)
    try:
        ...
        started = ai_invoke_review.run_review_gate(group_id, pending, run)
    except HTTPException as exc:
        logger.warning("ai-invoke auto-resume rejected for %s: %s",
                       group_id, getattr(exc, "detail", exc))
        svc._park_handoff(run, pending, HOP_HANDOFF_FAILED_STOP_CODE)
        return
    except Exception:
    ...
    svc._clear_handoff_row(group_id, run.get("run_id"))
...
def resume_chain(
    *, group_id: str, user_id: str, api_base_url: str, locale: str = "ko",
    ...
    with _group_resume_lock(group_id):
        active = svc._active_run_for_group(group_id)
        if active is not None:
        ...
        preflight = svc._paused_row_resume_state(project_id, pre_row, include_target=True)
        ...
        try:
            ...
            next_seq = svc._next_incomplete_item_seq(row["doc_ref"])
        except Exception:
            _restore_row()
        ...
        reviewer_overrides = ai_invoke_review._resumable_reviewer_overrides(
        ...
        try:
            gate = ai_invoke_review.resolve_review_gate(gate_bundle)
        except Exception:
            _restore_row()
        ...
        if gate.get("stage") in (REVIEW_HOP_KIND, REWORK_HOP_KIND):
            ...
            ai_invoke_review._queue_gate_bundle(group_id, queued)
            spawn = svc._spawn_review_hop if gate["stage"] == REVIEW_HOP_KIND else svc._spawn_rework_hop
            try:
                return spawn(group_id, queued, gate)
            except HTTPException as exc:
                svc.clear_auto_resume(group_id)
                _restore_row()
...
def _handoff_row_in_flight(row: dict) -> bool:
    ...
    if (row.get("stop_code") or "") != HOP_HANDOFF_STOP_CODE:
        return False
    if svc._active_run_for_group(row.get("group_id") or "") is not None:
        return True
...
def active_all(user_id: str) -> dict:
    ...
    with _runs_lock:
        candidates = [
            run for run in svc._runs.values()
    ...
    for row in rows:
        ...
        if stale:
            ...
            continue
        resume_state = svc._paused_row_resume_state(row["group_id"].split(".")[0], row)
        paused.append({
            "group_id": row["group_id"],
            "doc_ref": row["doc_ref"],
            ...
            "chain_docs_target": row.get("chain_docs_target"),
            "chain_docs_reached": int(row.get("chain_docs_reached") or 0),
            "pending_q_doc_ids": svc._open_q_doc_ids(row["group_id"]),
