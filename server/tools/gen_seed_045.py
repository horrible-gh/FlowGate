# -*- coding: utf-8 -*-
"""Generator for 045_seed_global_design_templates.sql (TR0016 rev4).

R0001 requirement: the program absorbs the 'long-used format' of the D/P/L/DB design
documents (the 3 legacy templates below) and provides it at authoring time, while
① reworking it to fit FlowGate, ② not reusing the original paths verbatim, and
③ providing the Korean-baseline original in each i18n language (ko/ja/en).

Legacy originals (absorption targets; the paths are not embedded in the bodies):
  Design     template_design.md
  Protocol   template_protocol_design.md
  Logic      template_logic.md
DB design has no legacy template, so it is newly written in the same format system (FlowGate-native).

This script is not an artifact but the 045 SQL generator. It holds 12 bodies (4 types × 3 locales)
and emits safe INSERT statements by escaping single quotes as ''.
"""
from pathlib import Path

# ─────────────────────────────────────────────────────────────────────────────
# Body definitions — BODIES[type_code][locale]
# Preserve the legacy originals' 'format' (decision-criteria table / document structure /
# per-section authoring criteria / checklist), and rework the domain-dependent (poker) examples
# into FlowGate-neutral, generic software examples.
# Do not put file-path tokens ( X:\ , _old/templates, etc.) into any body (AC-1 / contains_path guard).
# ─────────────────────────────────────────────────────────────────────────────
...
# ── Protocol (P) — absorbs template_protocol_design.md ───────────────────────────
P_KO = """# {시나리오 묶음명} 프로토콜설계 (P)
...
P_EN = """# {scenario group} Protocol Design (P)
...
# ── Logic (L) — absorbs template_logic.md ───────────────────────────────────────
L_KO = """# {모듈명} 로직설계 (L)
...
# ── DB — no legacy template. Newly written FlowGate-native in the same format system ──────
DB_KO = """# {대상 영역} DB설계 (DB)
...
DB_EN = """# {target area} DB Design (DB)
...
BODIES = {
    "D":  {"ko": D_KO,  "ja": D_JA,  "en": D_EN},
    "P":  {"ko": P_KO,  "ja": P_JA,  "en": P_EN},
    "L":  {"ko": L_KO,  "ja": L_JA,  "en": L_EN},
    "DB": {"ko": DB_KO, "ja": DB_JA, "en": DB_EN},
}
TYPE_ORDER = ["D", "P", "L", "DB"]
LOCALE_ORDER = ["ko", "ja", "en"]
STAMP = "2026-06-14T00:00:00+09:00"

# ── path-non-exposure self-check (same rule as the AC-1 / contains_path guard) ───────
import re
_DRIVE = re.compile(r"(^|\s)[A-Za-z]:[\\/]")
_TPL = re.compile(r"^\s*Template:\s*\S", re.MULTILINE)


def has_path(t: str) -> bool:
    return bool(_DRIVE.search(t) or "_rule/templates" in t or "_old/templates" in t or _TPL.search(t))
...
def main() -> None:
    for tc in TYPE_ORDER:
        for loc in LOCALE_ORDER:
            body = BODIES[tc][loc]
            assert not has_path(body), f"path token in {tc}/{loc}"  # AC-1 self-guard

    out = []
    out.append("-- 045_seed_global_design_templates.sql")
    out.append("-- flowgate.default.0024.0015-T  (design: D0010 / P0011 / P0012 / L0013 / DB0014)")
    out.append("-- Stage 3-3: SEED the GLOBAL standard template body for every design type")
    out.append("-- (D/P/L/DB) in EVERY supported locale (ko/ja/en).")
    out.append("--")
    out.append("-- R0001 requirement: FlowGate should ingest the long-used legacy D/P/L/DB formats (template_design /")
    out.append("-- template_protocol_design / template_logic) and offer them during document authoring,")
    out.append("-- while adapting them for FlowGate, keeping file paths out of the body (AC-1), and providing")
    out.append("-- the Korean source and i18n variants (ko/ja/en). DB design has no legacy template, so it is")
    out.append("-- newly written in the same format as a FlowGate-native template.")
    out.append("--")
    out.append("-- WHY GLOBAL: 043/044 only create empty registry/body tables. Without rows, resolution")
    out.append("-- permanently falls back to rank-5 type-default skeletons and only reports that no standard template is registered.")
    out.append("-- Load global standard bodies (project_id IS NULL) so every project immediately receives real content")
    out.append("-- through global-exact resolution without manual registration or redeploy. Project-specific bodies (E4/E5)")
    out.append("-- still win through project-first ordering (DB0014 §4-2).")
    out.append("--")
    out.append("-- IDEMPOTENT: registry rows rely on uq_dtt_global_type (044), and body rows rely on PK(template_id, locale).")
    out.append("-- INSERT OR IGNORE makes reapply/rerun a no-op. Keep template_path NULL.")
    out.append("-- The D option body lives in document_type_template_contents; AC-1 keeps paths hidden.")
    out.append("-- Generated by tools/gen_seed_045.py; that is the single source of truth for bodies. Do not hand-edit.")
    out.append("-- Rollback = DELETE FROM document_type_templates WHERE project_id IS NULL (CASCADE).")
    out.append("")
    out.append("PRAGMA foreign_keys = OFF;")
    out.append("BEGIN;")
    out.append("")
    out.append("-- 1. Global registry rows (<=1 per type via uq_dtt_global_type) ----------------")
    out.append("INSERT OR IGNORE INTO document_type_templates")
    out.append("    (project_id, type_code, template_path, is_active, uploaded_by, uploaded_at)")
    out.append("VALUES")
    rows = ",\n".join(
        f"    (NULL, '{tc}', NULL, 1, NULL, '{STAMP}')" for tc in TYPE_ORDER
    )
    out.append(rows + ";")
    out.append("")
    out.append("-- 2. Per-locale standard bodies (ko/ja/en), joined to each global row ----------")
    for tc in TYPE_ORDER:
        for loc in LOCALE_ORDER:
            body = sq(BODIES[tc][loc])
            out.append(
                "INSERT OR IGNORE INTO document_type_template_contents\n"
                "    (template_id, locale, content, updated_by, updated_at)\n"
                f"SELECT id, '{loc}',\n'{body}',\n"
                f"       NULL, '{STAMP}'\n"
                f"FROM document_type_templates WHERE project_id IS NULL AND type_code = '{tc}';"
            )
            out.append("")
    out.append("COMMIT;")
    ...
    dest = Path(__file__).resolve().parents[1] / "sql" / "migrations" / "sqlite" / "045_seed_global_design_templates.sql"
    dest.write_text("\n".join(out), encoding="utf-8")
    print(f"wrote {dest}  ({dest.stat().st_size} bytes)")
    # quick byte sizes per body
    for tc in TYPE_ORDER:
        sizes = {loc: len(BODIES[tc][loc].encode("utf-8")) for loc in LOCALE_ORDER}
        print(tc, sizes)
