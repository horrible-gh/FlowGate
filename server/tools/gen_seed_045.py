...
def main() -> None:
    ...
    out.append("--")
    out.append("-- R0001 요건: D/P/L/DB 의 '예전부터 쓰던 레거시 포맷'(template_design /")
    out.append("-- template_protocol_design / template_logic)을 프로그램이 흡수해 작성 시 제공하되,")
    out.append("-- 플로게이트에 맞게 개편하고, 파일 경로를 본문에 싣지 않으며(AC-1), 한글 기준 원본을")
    out.append("-- i18n(ko/ja/en) 각 언어로 제공한다. DB설계는 레거시 템플릿이 없어 동일 포맷 체계로")
    out.append("-- 신규 작성(플로게이트 네이티브).")
    out.append("--")
    out.append("-- WHY GLOBAL: 043/044 는 빈 레지스트리/본문 테이블만 만든다. 행이 없으면 resolution")
    out.append("-- 이 영구히 rank-5(type-default 골격)로 떨어져 '표준 템플릿 미등록'만 나온다. 전역")
    out.append("-- (project_id IS NULL) 표준 본문을 적재해 모든 프로젝트가 즉시 global-exact 로 실제")
    out.append("-- 본문을 받는다(수동 등록·재배포 불요). 프로젝트 전용 본문(E4/E5)은 project-first")
    out.append("-- 정렬로 여전히 우선한다(DB0014 §4-2).")
    out.append("--")
    out.append("-- IDEMPOTENT: 레지스트리는 uq_dtt_global_type(044), 본문은 PK(template_id, locale)에")
    out.append("-- 기대어 INSERT OR IGNORE. 재적용/재실행은 무위(no-op). template_path 는 NULL 유지")
    out.append("-- (D안 본문은 document_type_template_contents 보유, AC-1 경로 미노출).")
    out.append("-- 이 파일은 tools/gen_seed_045.py 로 생성됨(본문 단일 진실은 거기). 직접 손편집 금지.")
