...
def create_user(
    ...
    # Assign the __SYSTEM__ project role
    conn.execute(
        """INSERT OR REPLACE INTO user_project_roles
               (user_id, project_id, role_id, granted_at)
           VALUES (?, ?, ?, ?)""",
        (user_id, SYSTEM_PROJECT, role_id, now),
    )
    conn.commit()
...
def list_users(conn: sqlite3.Connection) -> list[dict]:
    rows = conn.execute(
        """SELECT u.user_id, u.username, u.email, u.is_admin, u.is_active, u.created_at,
                  GROUP_CONCAT(upr.role_id) as roles
           FROM users u
