...
VALUES
    (NULL, 'CH', '대화', 'general', 1, 1, 25, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING;
...
SELECT id, 'ko', '대화'
FROM   document_types
WHERE  series = 'general' AND type_code = 'CH' AND project_id IS NULL ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
DO $fg_or_ignore$
