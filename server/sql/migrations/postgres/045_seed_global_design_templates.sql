...
-- 기대어 INSERT. 재적용/재실행은 무위(no-op). template_path 는 NULL 유지
...
VALUES
    ...
    (NULL, 'DB', NULL, 1, NULL, '2026-06-14T00:00:00+09:00') ON CONFLICT DO NOTHING;
...
FROM document_type_templates WHERE project_id IS NULL AND type_code = 'D' ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
DO $fg_or_ignore$
...
',
       NULL, '2026-06-14T00:00:00+09:00'
FROM document_type_templates WHERE project_id IS NULL AND type_code = 'P' ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
DO $fg_or_ignore$
...
',
       NULL, '2026-06-14T00:00:00+09:00'
FROM document_type_templates WHERE project_id IS NULL AND type_code = 'L' ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
DO $fg_or_ignore$
...
',
       NULL, '2026-06-14T00:00:00+09:00'
FROM document_type_templates WHERE project_id IS NULL AND type_code = 'DB' ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
DO $fg_or_ignore$
