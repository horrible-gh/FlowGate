...
-- ── 3. Swap ───────────────────────────────────────────────────────────────────
-- [pg-fk-rebuild] preserve inbound FOREIGN KEYs across the drop+recreate of "document_type_templates"
DO $$
DECLARE _stmt text;
BEGIN
    CREATE TEMP TABLE _fk_rb_document_type_templates ON COMMIT DROP AS
            ...
            WHERE con.contype = 'f' AND con.confrelid = to_regclass('document_type_templates')
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_dtt_global_type
    ON document_type_templates(type_code) WHERE project_id IS NULL;

-- [pg-fk-rebuild] restore inbound FOREIGN KEYs for "document_type_templates"
DO $$
DECLARE _stmt text;
BEGIN
    IF to_regclass('pg_temp._fk_rb_document_type_templates') IS NOT NULL THEN
        FOR _stmt IN SELECT stmt FROM _fk_rb_document_type_templates LOOP
            EXECUTE _stmt;
        END LOOP;
    END IF;
END $$;
