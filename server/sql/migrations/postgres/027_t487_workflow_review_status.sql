...
FROM documents;
-- [pg-fk-rebuild] preserve inbound FOREIGN KEYs across the drop+recreate of "documents"
DO $$
DECLARE _stmt text;
BEGIN
    CREATE TEMP TABLE _fk_rb_documents ON COMMIT DROP AS
            ...
            WHERE con.contype = 'f' AND con.confrelid = to_regclass('documents')
...
-- [pg-fk-rebuild] restore inbound FOREIGN KEYs for "documents"
DO $$
DECLARE _stmt text;
BEGIN
    IF to_regclass('pg_temp._fk_rb_documents') IS NOT NULL THEN
        FOR _stmt IN SELECT stmt FROM _fk_rb_documents LOOP
            EXECUTE _stmt;
        END LOOP;
    END IF;
END $$;
