...
VALUES ('perm_test_run', '테스트 실행', 'TS 문서의 원격 테스트 실행 권한', CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING;
...
WHERE permission_id = 'perm_document_approve' ON CONFLICT DO NOTHING;
EXCEPTION WHEN check_violation OR not_null_violation OR foreign_key_violation THEN
    NULL;  -- OR IGNORE: drop the violating row(s), like SQLite/MySQL
END $fg_or_ignore$;
