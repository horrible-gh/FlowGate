...
-- (NR0010 §1-1). `tr_commit_ledger` is the one persistent store the approval hook writes and
-- the workflow strip / Git status panel / time-machine preview read.
--
...
-- deleted — a cancel writes `state='canceled'` onto the row it cancels (D0005 K5). The only
-- deletion path is the CASCADE from documents/groups disappearing (DB0008 §5-1).
--
-- `group_id` references `groups`, NOT `group_git_state`: a git-inactive group still gets a
-- `no_commit` row and has no git-state row at all (DB0008 §2-1 / §5-2).
--
-- No `seq` / `doc_code` columns on purpose — both are derived from `documents` and seq moves
-- when the workflow is edited (D0005 K9); every read joins `documents` (DB0008 §4-6).
--
...
--
-- Deliberate deviation from the DB0008 §3-4 block, postgres only: the three ADD COLUMNs carry
-- `IF NOT EXISTS`. That guard was left off for MySQL because MySQL cannot be relied on to
-- support it; postgres has supported it since 9.6, and the file is strictly safer with it.
--
...
ALTER TABLE group_git_state ADD COLUMN IF NOT EXISTS last_cancel_block_reason TEXT;
ALTER TABLE group_git_state ADD COLUMN IF NOT EXISTS last_cancel_block_sub    TEXT;
ALTER TABLE group_git_state ADD COLUMN IF NOT EXISTS last_cancel_block_at     TEXT;
