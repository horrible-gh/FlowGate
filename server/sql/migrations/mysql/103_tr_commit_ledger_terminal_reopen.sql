...
--
-- MySQL-only, and the same premise 087 wrote down: `ADD COLUMN` carries no `IF NOT EXISTS`
-- (unavailable before 8.0.29). Re-application safety rests on the migrations ledger keying on
-- the filename, which applies this file exactly once.
--
-- Additive only. Rollback is `ALTER TABLE tr_commit_ledger DROP COLUMN reopened_terminal_at`.

ALTER TABLE tr_commit_ledger ADD COLUMN reopened_terminal_at TEXT NULL;
