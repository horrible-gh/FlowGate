...
-- flowgate.default.0486 T0029 item 2 (NR0028 F2): rebuild the current 106 shape because
-- MySQL's unnamed CHECK name is not portable; preserve data, foreign keys, and indexes while
-- widening stop_reason by exactly one value ('review_stalled').
CREATE TABLE ai_invoke_document_review_loops_new (
 ...
 stop_reason VARCHAR(32) NULL CHECK (stop_reason IS NULL OR stop_reason IN ('review_passed','review_count_exhausted','retry_exhausted','total_timeout','review_verdict_hold','restart_orphaned','review_stalled')), stop_detail TEXT NULL,
 ...
 attempts_used INTEGER NOT NULL DEFAULT 0 CHECK (attempts_used >= 0), created_at TEXT NOT NULL, updated_at TEXT NOT NULL, card_dismissed_at VARCHAR(64) NULL,
 CONSTRAINT fk_aidrl_group_stalled FOREIGN KEY(group_id) REFERENCES groups(group_id) ON DELETE CASCADE,
 CONSTRAINT fk_aidrl_doc_stalled FOREIGN KEY(doc_ref) REFERENCES documents(doc_id) ON DELETE CASCADE,
 CONSTRAINT fk_aidrl_reviewer_stalled FOREIGN KEY(reviewer_provider_id) REFERENCES ai_providers(provider_id) ON DELETE RESTRICT,
 CONSTRAINT fk_aidrl_rework_stalled FOREIGN KEY(rework_provider_id) REFERENCES ai_providers(provider_id) ON DELETE RESTRICT,
