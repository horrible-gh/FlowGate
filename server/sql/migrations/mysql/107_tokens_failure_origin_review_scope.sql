...
ALTER TABLE tokens RENAME TO tokens_before_failure_origin_scope;
CREATE TABLE tokens (
    ...
    doc_ref TEXT, action_scope TEXT NOT NULL CHECK (action_scope IN ('new', 'edit', 'workflow_decide', 'review', 'test_run', 'workflow_sequence_edit', 'resolve_conflict', 'chat', 'failure_origin_review')),
    ...
    failure_origin_target_run_id TEXT, failure_origin_before_marker TIMESTAMP NULL
);
...
SELECT token_id, hash, pepper_id, project, group_id, doc_ref, action_scope, issued_to,
    ...
    continuation_target_seq, continuation_review_mode, continuation_locale, merge_id,
    continuation_instruction_mode, provider_id, ai_run_id,
    continuation_auto_approve_item_seqs, revoke_claim FROM tokens_before_failure_origin_scope;
DROP TABLE tokens_before_failure_origin_scope;
