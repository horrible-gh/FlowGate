...
-- the old name.
--
-- MySQL-only, and the same premise 086 wrote down: `ADD COLUMN` carries no `IF NOT EXISTS`
-- (that syntax is not available before 8.0.29). Re-application safety rests on the migrations
-- ledger keying on the filename, which applies this file exactly once — the file itself is NOT
-- idempotent on MySQL, so it must never be renamed WITHOUT a `migration_renames.RENAMES` entry
-- (see [[renamed-migration-reruns-on-old-dbs]]) — the 086 -> 087 move above has one.
--
-- Additive only. Rollback is `ALTER TABLE tr_commit_ledger DROP COLUMN restored_from_id`.

ALTER TABLE tr_commit_ledger ADD COLUMN restored_from_id INT NULL;
