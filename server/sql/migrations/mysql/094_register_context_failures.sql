...
--   * the FK is run_id -> ai_invoke_runs(run_id), not a UUID `id` column: ai_invoke_runs
--     is keyed on the run_id and has no UUID PK. Rows are therefore written in the same
...
--
-- MySQL specifics: the indexed/unique columns are VARCHAR (a TEXT key needs a prefix
-- length), the three indexes live inside the table body because MySQL has no
-- CREATE INDEX IF NOT EXISTS, and BOOLEAN is TINYINT(1) as in 091.
CREATE TABLE IF NOT EXISTS register_context_failures (
    ...
    boundary              VARCHAR(32)  NOT NULL CHECK (boundary IN ('register_dispatch', 'inbox', 'legacy_unclassified')),
    ...
    ai_run_id             TEXT,
    axis_first_mismatch   VARCHAR(16)  CHECK (axis_first_mismatch IS NULL OR axis_first_mismatch IN ('action', 'project', 'group', 'doc')),
    axes_all_mismatches   TEXT,
    token_id_hash         TEXT,
    expected_fingerprint  TEXT,
    actual_fingerprint    TEXT,
    binding_relaxed       TINYINT(1)   NOT NULL DEFAULT 0 CHECK (binding_relaxed IN (0, 1)),
    relaxed_axis          VARCHAR(16),
    status                INTEGER,
    code                  TEXT,
    reason                TEXT,
    turn                  INTEGER,
    notes                 TEXT,
    CONSTRAINT fk_rcf_run FOREIGN KEY (run_id) REFERENCES ai_invoke_runs(run_id) ON DELETE CASCADE,
    ...
    INDEX idx_rcf_run_time (run_id, recorded_at DESC),
    INDEX idx_rcf_boundary_axis (boundary, axis_first_mismatch),
    INDEX idx_rcf_recorded (recorded_at DESC)
);
