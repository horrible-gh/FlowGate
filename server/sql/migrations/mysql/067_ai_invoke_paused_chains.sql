...
CREATE TABLE IF NOT EXISTS ai_invoke_paused_chains (
    ...
    group_id                VARCHAR(191)    NOT NULL UNIQUE REFERENCES groups(group_id) ON DELETE CASCADE,
    doc_ref                 VARCHAR(191)    NOT NULL REFERENCES documents(doc_id) ON DELETE CASCADE,
    mode                    TEXT    NOT NULL DEFAULT ('continuous') CHECK (mode = 'continuous'),
    paused_by               VARCHAR(191)    NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
