...
-- flowgate.default.0486 T0019: rebuild the current 105 shape because MySQL's unnamed
-- CHECK name is not portable; preserve data, foreign keys, and indexes.
CREATE TABLE ai_invoke_document_review_loops_new (
 ...
 stop_reason VARCHAR(32) NULL CHECK (stop_reason IS NULL OR stop_reason IN ('review_passed','review_count_exhausted','retry_exhausted','total_timeout','review_verdict_hold','restart_orphaned')), stop_detail TEXT NULL,
