...
CREATE TABLE IF NOT EXISTS questions (
    ...
    q_id        VARCHAR(191)    NOT NULL UNIQUE,
    -- Numbering format: Q0001, Q0002, ...

    project_id  VARCHAR(191)    REFERENCES projects(project_id) ON DELETE SET NULL,
    -- For project-specific Q lookups (NULL = global)

    title       TEXT    NOT NULL,
    -- Overall Q title/summary

    created_by  VARCHAR(191)    NOT NULL REFERENCES users(user_id),
    -- Worker who registered the question

    pm_id       VARCHAR(191)    REFERENCES users(user_id),
...
CREATE TABLE IF NOT EXISTS answers (
    ...
    answered_by VARCHAR(191)    NOT NULL REFERENCES users(user_id),
    -- Answer author (PM)
