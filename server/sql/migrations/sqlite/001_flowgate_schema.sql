...
INSERT OR IGNORE INTO roles (role_id, role_name, description, is_system, created_at, updated_at) VALUES
    ('role_admin',    '관리자', '시스템 전체 관리',   1, datetime('now'), datetime('now')),
    ('role_manager',  '매니저', '프로젝트 관리',       1, datetime('now'), datetime('now')),
    ('role_worker',   '작업자', '일반 작업',           1, datetime('now'), datetime('now')),
    ('role_viewer',   '뷰어',   '읽기 전용',           1, datetime('now'), datetime('now'));

-- 27 permissions
INSERT OR IGNORE INTO permissions (permission_id, permission_name, created_at) VALUES
    ('perm_project_create',         '프로젝트 생성',         datetime('now')),
    ('perm_project_read',           '프로젝트 조회',         datetime('now')),
    ('perm_project_update',         '프로젝트 수정',         datetime('now')),
    ('perm_project_delete',         '프로젝트 삭제',         datetime('now')),
    ('perm_user_create',            '사용자 생성',           datetime('now')),
    ('perm_user_read',              '사용자 조회',           datetime('now')),
    ('perm_user_update',            '사용자 수정',           datetime('now')),
    ('perm_user_delete',            '사용자 삭제',           datetime('now')),
    ('perm_role_assign',            '역할 부여',             datetime('now')),
    ('perm_role_revoke',            '역할 회수',             datetime('now')),
    ('perm_document_create',        '문서 생성',             datetime('now')),
    ('perm_document_read',          '문서 조회',             datetime('now')),
    ('perm_document_update',        '문서 수정',             datetime('now')),
    ('perm_document_delete',        '문서 삭제',             datetime('now')),
    ('perm_document_approve',       '문서 승인',             datetime('now')),
    ('perm_document_reject',        '문서 반려',             datetime('now')),
    ('perm_group_create',           '그룹 생성',             datetime('now')),
    ('perm_group_read',             '그룹 조회',             datetime('now')),
    ('perm_group_update',           '그룹 수정',             datetime('now')),
    ('perm_group_close',            '그룹 종료',             datetime('now')),
    ('perm_settings_read',          '설정 조회',             datetime('now')),
    ('perm_settings_update',        '설정 수정',             datetime('now')),
    ('perm_tv_run',                 'TV/TVR 실행',           datetime('now')),
    ('perm_tv_approve',             'TV/TVR 승인',           datetime('now')),
    ('perm_numbering_manage',       '채번 관리',             datetime('now')),
    ('perm_document_type_manage',   '문서 타입 관리',         datetime('now')),
    ('perm_storage_manage',         '스토리지 관리',         datetime('now'));
...
INSERT OR IGNORE INTO document_types (project_id, type_code, type_name, series, is_system, is_active, sort_order, created_at, updated_at) VALUES
    (NULL, 'R',  '요건정의',             'requirements', 1, 1, 10,  datetime('now'), datetime('now')),
    (NULL, 'M',  '메모',                 'requirements', 1, 1, 20,  datetime('now'), datetime('now')),
    (NULL, 'Q',  '질의',                 'requirements', 1, 1, 30,  datetime('now'), datetime('now')),
    (NULL, 'A',  '응답',                 'requirements', 1, 1, 40,  datetime('now'), datetime('now')),
    (NULL, 'L',  '로그',                 'requirements', 1, 1, 50,  datetime('now'), datetime('now')),
    (NULL, 'B',  '버그',                 'requirements', 1, 1, 60,  datetime('now'), datetime('now')),
    (NULL, 'DS', '설계지시',             'instruction',  1, 1, 10,  datetime('now'), datetime('now')),
    (NULL, 'N',  '조사지시',             'instruction',  1, 1, 20,  datetime('now'), datetime('now')),
    (NULL, 'T',  '작업지시',             'instruction',  1, 1, 30,  datetime('now'), datetime('now')),
    (NULL, 'TS', '테스트시나리오지시',   'instruction',  1, 1, 40,  datetime('now'), datetime('now')),
    (NULL, 'D',  '기본설계',             'design',       1, 1, 10,  datetime('now'), datetime('now')),
    (NULL, 'P',  '프로토콜',             'design',       1, 1, 20,  datetime('now'), datetime('now')),
    (NULL, 'L',  '로직',                 'design',       1, 1, 30,  datetime('now'), datetime('now')),
    (NULL, 'DB', '데이터베이스',         'design',       1, 1, 40,  datetime('now'), datetime('now')),
    (NULL, 'NR', '조사레포트',           'work',         1, 1, 10,  datetime('now'), datetime('now')),
    (NULL, 'TR', '작업레포트',           'work',         1, 1, 20,  datetime('now'), datetime('now')),
    (NULL, 'TSR','테스트레포트',         'work',         1, 1, 30,  datetime('now'), datetime('now')),
    (NULL, 'V',  '리뷰의뢰',             'work',         1, 1, 40,  datetime('now'), datetime('now')),
    (NULL, 'C',  '커밋',                 'work',         1, 1, 50,  datetime('now'), datetime('now')),
    (NULL, 'AC', '승인',                 'action',       1, 1, 10,  datetime('now'), datetime('now')),
    (NULL, 'RJ', '반려',                 'action',       1, 1, 20,  datetime('now'), datetime('now'));
...
INSERT OR IGNORE INTO system_settings (setting_key, setting_value, value_type, description, updated_at) VALUES
    ('storage_root',              '',    'string',  '스토리지 기본 루트 경로',          datetime('now')),
    ('log_retention_days',        '90',  'integer', '로그 보존 기간(일)',               datetime('now')),
    ('log_level',                 'INFO','string',  '로그 출력 레벨',                   datetime('now')),
    ('jwt_expiry_minutes',        '30',  'integer', 'JWT 액세스 토큰 만료(분)',         datetime('now')),
    ('refresh_token_expiry_days', '7',   'integer', '리프레시 토큰 만료(일)',           datetime('now')),
    ('support_email',             '',    'string',  '지원 문의 이메일 주소',            datetime('now')),
    ('jwt_expire_min',            '30',  'integer', 'JWT 액세스 토큰 만료(분) 별칭',   datetime('now')),
    ('refresh_expire_days',       '14',  'integer', '리프레시 토큰 만료(일) 별칭',     datetime('now'));
