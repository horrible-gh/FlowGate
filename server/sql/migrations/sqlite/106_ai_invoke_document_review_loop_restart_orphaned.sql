...
-- flowgate.default.0486 T0019: restart recovery terminally closes an active document
-- review loop. SQLite cannot alter the CHECK in place, so preserve the current 105
-- shape, data, foreign keys, and indexes while widening stop_reason.
...
CREATE TABLE ai_invoke_document_review_loops_new (
 ...
 stop_reason TEXT CHECK (stop_reason IS NULL OR stop_reason IN ('review_passed','review_count_exhausted','retry_exhausted','total_timeout','review_verdict_hold','restart_orphaned')),
 stop_detail TEXT,
