...
-- 정렬 순서를 인덱스가 담고 있어 LIMIT 이 조기 종료가 된다(070 이 documents 목록용으로
-- 만든 (project_id, group_id, updated_at) 인덱스와 같은 수법이다 — 이름을 그대로 적지
-- 않는 것은 070 이후 파일이 그 인덱스를 다시 정의하지 않는지 검사하는 가드 때문이다).
