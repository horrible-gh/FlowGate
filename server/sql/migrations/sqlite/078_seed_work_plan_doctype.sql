...
-- (NR0005 §6.1: DB design is 0 pages). Idempotent via INSERT OR IGNORE, so rerunning
...
VALUES
    (NULL, 'WP', '작업계획', 'general', 1, 1, 30, datetime('now'), datetime('now'));
...
SELECT id, 'ko', '작업계획'
FROM   document_types
WHERE  series = 'general' AND type_code = 'WP' AND project_id IS NULL;
...
FROM   document_types WHERE series = 'general' AND type_code = 'WP' AND project_id IS NULL;
